var _carbonads = {
    init: function(where, force_serve) {
        ...
        if (ignoretargeting)
            pro.src += '&ignoretargeting=' + ignore;
        if (forwardedip)
        ...
        if (day.length || life.length) {
            ...
            if (freqcap.length)
                pro.src += '&freqcap=' + encodeURIComponent(freqcap.join(';'))
        }
...
function _carbonads_go(b) {
    ...
    if (!_carbonads.nonempty(ad.statlink) && (!(_carbonads.nonempty(ad.fallbackImage) && _carbonads.nonempty(ad.fallbackLink) && _carbonads.nonempty(ad.fallbackTitle)) || !(_carbonads.nonempty(ad.fallbackZoneKey)))) {
        if (_carbonads.gotback)
            return;
        _carbonads.gotback = true;
        var fallback = document.createElement('script');
        fallback.type = 'text/javascript';
        fallback.id = '_carbonads_fallbackjs';
        fallback.src = '//srv.carbonads.net/ads/' + (_carbonads.nonempty(ad.fallbackZoneKey) ? ad.fallbackZoneKey : 'CK7DT53I') + '.json?segment=placement:' + placement + '&callback=_carbonads_go';
    ...
    el.innerHTML = '<span class="carbon-wrap"><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-img" target="_blank" rel="noopener"><img src="' + image + '" alt="" border="0" height="100" width="130" /></a><a href="' + fulllink.replace('[timestamp]', time) + '" class="carbon-text" target="_blank" rel="noopener">' + _carbonads.htmlEncode(description) + '</a></span>';
